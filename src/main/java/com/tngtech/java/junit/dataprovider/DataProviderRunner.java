...
import tngtech.java.junit.dataprovider.internal.DataConverter;
import tngtech.java.junit.dataprovider.internal.TestGenerator;
import tngtech.java.junit.dataprovider.internal.TestValidator;

/**
 * A custom runner for JUnit that allows the usage of <a href="http://testng.org/">TestNG</a>-like dataproviders. Data providers are public,
 * static methods that return an {@link Object}{@code [][]} (see {@link DataProvider}).
 * <p>
...
public class DataProviderRunner extends BlockJUnit4ClassRunner {
	...
	/**
	 * The {@link DataConverter} to be used to convert from supported return types of any dataprovider to {@link List} {@code <}
	 * {@link Object}{@code []>} such that data can be further handled.
	 * <p>
	...
	/**
	 * The {@link TestValidator} to be used to validate all test methods to be executed as test and all dataprovider to be used to explode
	 * tests.
	 * <p>
	 * This field is package private (= visible) for testing.
	 * </p>
	...
	/**
	 * The {@link TestGenerator} to be used to generate all framework methods to be executed as test (enhanced by data providers data if
	 * desired).
	 * <p>
	 * This field is package private (= visible) for testing.
	 * </p>
	 ...
	 *
	 * @param clazz
	 *            the test {@link Class} to run
	 * @throws InitializationError
	 *             if the test {@link Class} is malformed.
	 */
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Is copied from {@link BlockJUnit4ClassRunner#validateInstanceMethods} because {@link #computeTestMethods()} must not be called if
	 * validation already found errors!
	 */
	@Override
	@Deprecated
	 ...
	 *             if given {@code errors} is {@code null}
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Overridden due to {@code @}{@link BeforeClass} methods are already processed, see {@link #computeTestMethods()} and
	 * {@link #invokeBeforeClass()}. Just add a {@link Statement} which is processing potential caught {@link Throwable} while
	 * {@code @BeforeClass} methods have been executed before.
	 *
	...
	/**
	 * Generates the exploded list of methods that run tests. All methods annotated with {@code @Test} on this class and super classes that
	 * are not overridden are checked if they use a {@code @}{@link DataProvider} or not. If yes, for each row of the {@link DataProvider}s
	 * result a specific, parameterized test method will be added. If not, the original test method is added.
	 * <p>
	 * Additionally, caches the result as {@link #computeTestMethods()} is call multiple times while test execution by the JUnit framework
	 * (to validate, to filter, to execute, ...).
	 *
	 ...
	 * {@inheritDoc}
	 * <p>
	 * If possible the given {@code filter} is wrapped by {@link DataProviderFilter} to enable filtering of tests using a dataprovider.
	 *
	 * @param filter
	 *            the {@link Filter} to be wrapped or apply, respectively
	 */
	@Override
	...
	/**
	 * Runs all {@code @}{@link BeforeClass} methods as by using the result of {@link ParentRunner#withBeforeClasses(Statement)} and
	 * immediately invoke its {@link Statement#evaluate()}. If a {@link Throwable}s was thrown, it will be stored in {@link #failure} that
	 * it can be processed later in overridden {@link #withBeforeClasses(Statement)}.
	 * <p>
	...
	/**
	 * Generates the exploded list of test methods for the given {@code testMethods}. Each of the given {@link FrameworkMethod}s is checked
	 * if it uses a {@code @}{@link DataProvider} or not. If yes, for each line of the {@link DataProvider}s result a specific,
	 * parameterized test method will be added. If no, the original test method is added.
	 * <p>
	 * This method is package private (= visible) for testing.
	 * </p>
	 *
	 * @param testMethods
	 *            the original test methods
	 * @return the exploded list of test methods (never {@code null})
	 */
	...
	/**
	 * Returns the dataprovider method that belongs to the given test method or {@code null} if no such dataprovider exists or the test
	 * method is not marked for usage of a dataprovider
	 * <p>
	 * This method is package private (= visible) for testing.
	 * </p>
	 *
	 * @param testMethod
	 *            test method that uses a dataprovider
	 * @return the dataprovider or {@code null} (if dataprovider does not exist or test method does not use any)
	 * @throws IllegalArgumentException
	 *             if given {@code testMethod} is {@code null}
	 */
	FrameworkMethod getDataProviderMethod(FrameworkMethod testMethod) {
		...
		for (FrameworkMethod method : dataProviderLocation.getAnnotatedMethods(DataProvider.class)) {
			if (!"".equals(useDataProvider.value()) && method.getName().equals(useDataProvider.value())) {
				return method;
			} else if ("".equals(useDataProvider.value())) {
				if (method.getName().equals(testMethod.getName()) && method.getReturnType().equals(Object[][].class)) {
					return method;
				}
			}
